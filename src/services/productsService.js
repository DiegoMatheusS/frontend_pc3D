...
export function getProducts() {
  return apiFirst({
    ...
    transform: (payload) => extractList(payload, ['hardwares', 'produtos']).map(normalizeProduct),
  })
}
...
export function getProductById(id) {
  return apiFirst({
    ...
    transform: (payload) => normalizeProduct(payload?.produto || payload?.hardware || payload),
  })
}
