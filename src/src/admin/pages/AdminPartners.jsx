...
export default function AdminPartners() {
  ...
  async function submit(event) {
    ...
    try {
      const body = {
        ...
        logoUrl: String(form.logoUrl ?? '').trim() || (form.id ? null : undefined),
        site: String(form.site ?? '').trim() || (form.id ? null : undefined),
        dominio: String(form.dominio ?? '').trim() || (form.id ? null : undefined),
        programaAfiliados: Boolean(form.programaAfiliados),
        observacao: String(form.observacao ?? '').trim() || (form.id ? null : undefined),
  ...
  async function removeCurrentPartner() {
    if (!canWriteCatalog || !form.id || saving) return
    const confirmed = window.confirm(`Excluir o parceiro ${String(form.nome ?? '').trim() || '#' + form.id}? Se houver ofertas vinculadas, o backend pode impedir a exclusão.`)
    if (!confirmed) return
    setSaving(true)
    try {
      await adminService.offers.removePartner(form.id)
      toast.show('Parceiro excluído.', 'sucesso')
      setForm(EMPTY)
      await load()
    } catch (err) {
      toast.show(err.message || 'Não foi possível excluir o parceiro.', 'erro')
    } finally {
      setSaving(false)
    }
  }
  ...
  return <>
    ...
    {canWriteCatalog && <section className="admin-form-card" style={{ marginBottom: 18 }}><form onSubmit={submit}><section className="admin-form-section"><h2>{form.id ? 'Editar parceiro' : 'Novo parceiro'}</h2><div className="admin-form-grid"><div className="admin-field"><label>Nome</label><input className="admin-input" required value={form.nome} onChange={(e) => update('nome', e.target.value)} /></div><div className="admin-field"><label>Domínio</label><input className="admin-input" value={form.dominio} onChange={(e) => update('dominio', e.target.value)} placeholder="kabum.com.br" /></div><div className="admin-field"><label>Site</label><input className="admin-input" type="url" value={form.site} onChange={(e) => update('site', e.target.value)} /></div><div className="admin-field"><label>Logo URL</label><input className="admin-input" type="url" value={form.logoUrl} onChange={(e) => update('logoUrl', e.target.value)} /></div><div className="admin-field full"><label>Observação</label><textarea className="admin-textarea" value={form.observacao} onChange={(e) => update('observacao', e.target.value)} /></div><div className="admin-field"><label className="admin-switch"><input type="checkbox" checked={form.programaAfiliados} onChange={(e) => update('programaAfiliados', e.target.checked)} /> Programa de afiliados</label></div>{form.id && <div className="admin-field"><label className="admin-switch"><input type="checkbox" checked={form.ativo} onChange={(e) => update('ativo', e.target.checked)} /> Ativo</label></div>}</div></section><footer className="admin-form-footer">{form.id && <button className="btn btn-perigo" type="button" disabled={saving} onClick={removeCurrentPartner}>Excluir parceiro</button>}<button className="btn btn-secundario" type="button" onClick={() => setForm(EMPTY)}>Limpar</button><button className="btn btn-primario" disabled={saving} type="submit">{saving ? 'Salvando...' : 'Salvar parceiro'}</button></footer></form></section>}
